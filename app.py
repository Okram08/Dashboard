...
# Titre
st.title("📈 Dashboard de Trading Complet")
...
dirs = st.multiselect("📈 Filtrer par direction :", df["dir"].unique(), default=df["dir"].unique())
...
# Backtest simplifié
pnl_buy_only = df_filtered[df_filtered["dir"] == "BUY"]["closedPnl"].sum()
pnl_sell_only = df_filtered[df_filtered["dir"] == "SELL"]["closedPnl"].sum()

st.markdown("### 🔁 Backtest simplifié (hypothétique)")
col1, col2 = st.columns(2)
col1.metric("PnL si 100% BUY", f"{pnl_buy_only:.2f} $")
col2.metric("PnL si 100% SELL", f"{pnl_sell_only:.2f} $")
